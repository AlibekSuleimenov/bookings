...
// SearchAvailabilityByDatesByRoomID returns true if availability exists for room, and false if no availability
...
func (m *postgresDBRepo) GetRoomByID(id int) (models.Room, error) {
	...
	stmt := `select id, room_name, created_at, updated_at
...
func (m *postgresDBRepo) AllReservations() ([]models.Reservation, error) {
	...
	stmt := `select r.id, r.first_name, r.last_name, r.email, r.phone, r.start_date, r.end_date, r.room_id, 
...
// AllNewReservations return a slice of all new reservations
...
func (m *postgresDBRepo) GetRestrictionsForRoomByDay(roomID int, start, end time.Time) ([]models.RoomRestriction, error) {
	...
	stmt := `select id, coalesce(reservation_id, 0), restriction_id, room_id, start_date, end_date
             from room_restrictions where $1 < end_date and $2 >= start_date and room_id = $3`

	rows, err := m.DB.QueryContext(ctx, stmt, start, end, roomID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
